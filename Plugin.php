...
class AliOssForTypecho_Plugin implements Typecho_Plugin_Interface
{
    ...
    {
        Typecho_Plugin::factory('Widget_Upload')->checkHandle         = array('AliOssForTypecho_Plugin', 'checkHandle');
...
<li><span style="color:#F00">去除每个图片随机创建一个文件夹，但是没有是否存在的检测，上传时要确保文件不会重名.</span></li>
</ol>');
        $form->addInput($des);
        ...
        $syName = new Typecho_Widget_Helper_Form_Element_Text('syName', NULL, '', 
            _t('样式名称'), '请填写使用的样式，留空为不使用。可以使用默认规则和自定义规则。详细请去<a target="_blank" href="https://help.aliyun.com/document_detail/48884.html?spm=5176.8466084.0.0.61631450mKn6m3">阿里云文档</a>了解。');
        $form->addInput($syName);
    ...
    *public static function checkHandle($file)
    *{
    *    if (empty($file['name'])) {
     *       return FALSE;
      *  }
    ...
    *}
    ...
    public static function uploadHandle($file)
    {
       ...
       // 取消随机文件夹
        ...
        $relative_path = date('Y/m/d/')  . $file_origin_name;
    ...
    public static function attachmentHandle(array $content) {
        ...
        $userDir = $options->plugin('AliOssForTypecho')->userDir;
        $syName = $options->plugin('AliOssForTypecho')->syName;
        if ($cdnUrl == '') {
            $bucket_name = $options->plugin('AliOssForTypecho')->bucketName;
            ...
            return $cdnUrl . $userDir . $content['attachment']->path . $syName;
        }
    }
